...
public class CloudServersUKHttpApiModule extends NovaHttpApiModule {
   ...
   public Multimap<URI, URI> customAliases(Multimap<URI, URI> aliases) {
      Multimap<URI, URI> mutable = Multimaps.filterKeys(aliases, new Predicate<URI>() {
         @Override
         public boolean apply(URI input)
         {
            return !URI.create(ExtensionNamespaces.VOLUME_ATTACHMENTS).equals(input);
         }
      });

      ImmutableMultimap.Builder<URI, URI> custom = ImmutableMultimap.<URI, URI> builder().putAll(mutable);
      custom.put(URI.create(ExtensionNamespaces.VOLUME_ATTACHMENTS),
